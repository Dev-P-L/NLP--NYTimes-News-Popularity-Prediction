...
## POPULARITY PREDICTION of NY TIMES BLOG ARTICLES                        
...
A rate of **94 % AUC** has been reached using **Natural Language Processing** and an **ensemble Machine Learning solution** combining eXtreme Gradient Boosting and Random Forest. Combination has been tested with ROC curves on bootstrapped resamples. 
...
## II. FOREWORD to READERS and ACKNOWLEDGEMENTS
...
Let me also thank my friend Richard Reagan, former Associate General Counsel, JPMorgan Chase, N.A., for indefatigable questioning and discussion about domain, data science and editing. Interaction has been most supportive.  
...
The higher popularity rate of Saturday is also considered statistically significant with 131 blog articles. 
...
In the word cloud above can be seen tokens such as *fashion*, *obama*, *presid*, *senat*, *billion*, *bank*, *million*, *work*, etc. 
...
The table above is the last step in our evaluating the predictive power of the XGB and RF models thanks to bootstrapped resample information. First, AUCs have been collected on the 675 resamples from XGB and the 75 resamples from RF. Second, AUCs have been regrouped according to parameterization, with 27 parameter value combinations for XGB and 3 parameter values for RF. Third, in the table above figure only AUCs corresponding to optimal parameterization. In the three steps, XGB has proved a little more performing in AUC. In the third and last step in particular, XGB scores an AUC of 93 % versus an AUC of 92 % for RF. 
...
As far as model optimization is concerned, an insight had previously been garnered: in the graphs of AUC polygons, a substantial difference had been noticed in the probability threshold that equaled true positive rate and false positive rate. Differences in results can be an insight towards combining models into an ensemble solution. 
...
The following table shows the AUC for XGB, for RF and for the ensemble solution. 
...
Predictions on the validation set are close to predictions on bootstrapped resamples with optimal parameterization: ranking is the same, AUC levels are similar for XGB and the ensemble solution and one percentage point higher for Random Forest.  
