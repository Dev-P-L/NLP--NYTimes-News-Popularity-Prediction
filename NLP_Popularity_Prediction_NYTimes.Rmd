...
graph <-  tab %>%
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
        ...
        axis.text.y = element_text(size = 12))
graph
rm(s, row.to.remove, df, v, tab, graph)
```
...
```{r Time decomposition of the time series PubDate, echo = T}
temp <- trainNews %>% 
...
There is a clear difference between weekend and weekdays outside weekends: average popularity rate is substantially higher during weekend, and especially on Sundays. But are these higher rates statistically representative? It is representative for Sunday, with at least200 blog articles. But what about Saturday?  
...
The higher popularity rate of Saturday is also considered statistically significative with 131 blog articles. 
...
Let's have a look at a wordcloud based on the bag of words to get some pre-attentive insights. 
...
In the wordcloud above can be seen tokens such as *fashion*, *obama*, *presid*, *senat*, *billion*, *bank*, *million*, *work*, etc. 
...
## VI. MACHINE LEARNING PREDICTION on the  TRAINING SET with XGB and RF

<br>

```{r Running  XGB and RF on training set}
...
Let's compare rf and xgb merits. It is only fitting that the first examined objects are the respective ROC curves and ROC AUCs. Let's start with XGB.
...
## VII. TESTING XGB and RF on BOOTSTRAPPED RESAMPLE DISTRIBUTIONS
...
But before retrieving insights from bootstrapped resample distributions, let's notice another subtle clue. At the upper right-hand corner of the AUC polygons is an indication of the probability threshold for which true positive rate and false negative rate are equal to each other. That threshold is pretty different for RF and for XGB: 0.452 for RF and 0.299 for XGB. Independently of performance, this gap in internal results might become an impactful insight in case there is heavy overfitting. Indeed, combining models with different results may boost performance... Let's keep that in mind and go back to bootstrapped resample distributions. 

Let's get started with a quick summary statistic per model, which is calculated accross all AUCs per model, i.e. accross 675 AUCs for XGB and 75 AUCs for R. This summary statistic gives 
...
Second, order in performance metric has been reversed: while RF delivers a perfect AUC of 100 % on the final model and slightly surpasses XGB, on resamples RF is slightly outperformed by XGB. This is especially so in minimum AUC and in average AUC. Would this insight be confirmed by density functions? 27 density functions will be drawn for XGB, i.e. one per each of the 27 combinations of parameter values; 3 density functions will be produced for RF, i.e. one per each of the 3 parameter values tuned on RF. Here are the XGB density functions. 
...
In two cases out of three, the top of the density function (and the main part of it as well) is below 92 %. In one case out of three, the top almost reaches 92 %. This contrasts with XGB having the top its 27 density functions above 92 %. 

For each density function, the average AUC (over 25 resamples) will be calculated and the following table will give the range between the minimum AUC and the maximum AUC per model; consequently, for model XGB, it will be the range over 27 average AUCs and, for model RF, it will be the range over 3 average AUCs.
...
The XGB range lies somewhat higher than the RF range. This is an additional insight towards the hypothesis of XGB performing somewhat better in this context.

From these ranges, let's extract, for each model, the AUC corresponding to the optimal parameter value(s). (For the record, they are averages over 25 resamples.)
...
The table above is the last step in our evaluating the predictive power of the XGB and RF models thanks to bootsrapped resample information. First, AUCs have been collected on the 675 resamples from XGB and the 75 resamples from RF. Second, AUCs have been regrouped according to parameterization, with 27 parameter value combinations for XGB and 3 parameter values for RF. Third, in the table above figure only AUCs corresponding to optimal parameterization. In the three steps, XGB has proved a little more performant in AUC. In the third and last step in particular, XGB scores an AUC of 93 % versus an AUC of 92 % for RF. 

At this stage, XGB should be the model to be validated on the validation set.

But AUC level has dramatically dropped with respect to the final model on the training set. Instead of being at 100 % or 99 %, AUC is now at 93-92 % on resamples and this lower level is most probably closer to predictive capability on the validation set. This drop in AUC looks like a stimulus to further modelling towards AUC optimization.  

<br>

## VIII. TESTING ENSEMBLE SOLUTION on BOOTSRAPPED RESAMPLE DISTRIBUTIONS

<br>

As far as model optimization is concerned, an insight had previously been garnered: in the graphs of AUC polygons, a substantial difference had been noticed in the probability threshold that equalled true positive rate and false negative rate. Differences in results can be an insight towards combining models into an ensemble solution. 

That is exactly what will be tried. An ensemble solution will be built by combining probabilities of the positive class originating from the two individual models: for each blog article, popularity will be predicted using the average of the probilities produced respectively by XGB and RF.
...
```{r Testing ensemble solution on the bootstrapped resamples, echo = TRUE}
# Getting values from optimal parameterization.
bt_xgb <- fit_xgb$bestTune
bt_rf <- fit_rf$bestTune

# Getting probabilities of positive class with parameterizing optimally and averaging on the 25 bootstrapped resamples.
df_xgb <- fit_xgb$pred %>% 
          as.data.frame(stringsAsFactors = FALSE) %>%
...
Let's have a look at the AUC produced by the ensemble solution, compared with AUC from XGB and RF. 
...
On the basis of bootstrapped resamples, the most performing model in AUC is the ensemble solution, which is consequently that will be hopefully validated on the validation set. 
...
```{r Building up the validation set}
back_up <- valNews
valNews <- back_up %>% 
  mutate(date = as.POSIXct(PubDate)) %>%
