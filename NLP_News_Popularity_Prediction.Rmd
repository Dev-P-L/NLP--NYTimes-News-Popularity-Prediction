...
- and NLP_News-Popularity-Prediction.pdf (idem as previous but in PDF format).
...
tab <- data.frame(matrix(c(overall_AUC_min_rf, overall_AUC_min_xgb,
                           overall_AUC_avg_rf, overall_AUC_avg_xgb,
                           overall_AUC_max_rf, overall_AUC_max_xgb),
...
Second, order in performance metric has been partially flipped between both models: *Random Forest*, which delivers an almost perfect AUC of almost 100 % on the final model on the training set, still comes first in minimum resample AUC but is slightly below in average resample AUC and is half a percentage point below in maximum resample AUC, taking into account all resample AUCs, i.e. 75 for *Random Forest* and 675 for *eXtreme Gradient Boosting*. 
...
- it corresponds to the parameter values 0.5 for *alpha*, 27 for *nrounds* and 0.9 for *lambda*;
...
For the record, AUC decreases all the way down the line for *Random Forest* and it might be asked whether it would not further increase for lower values of *mtry*. This has been tested and it would not improve AUC, on the contrary. 
...
tab <- rbind(density_rf, density_xgb) %>% 
...
synthesis <- rbind(auc_xgb_rf, auc_ensemble) %>%
    `colnames<-`("auc") %>%
    mutate(name = c("Random Forest Model",
                    "eXtreme Gradient Boosting Model",
...
final <- c(auc_rf, auc_xgb, auc_ens) %>%
  as.data.frame() %>%
  `colnames<-`("auc") %>%
  mutate(mod = c("Random Forest Model",
                 "eXtreme Gradient Boosting Model",
                 "Ensemble Solution XGB + RF")) %>%
  select(mod, auc) %>%
...
- AUCs on the validation set are practically the same as average AUCs calculated with optimal parameterization on the bootstrapped resamples from the training set;
